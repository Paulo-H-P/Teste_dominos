class FluxoCompraPage {
    ...
    elements = {
        ...
        // Seletores para o botão de salvar/escolher sabor (múltiplas estratégias)
        ...
        adicionarCarrinho: () => cy.get('.btn-primary'),
        seguirCarrinho: () => cy.get('.btn-outline-red'),
        pagamento: () => cy.get('.btn-primary'),
        ...
        cy.get('body').then(($body) => {
            ...
            if (temBanner) {
                ...
                cy.get('app-cookie-banner').then(($banner) => {
                    const botaoFechar = $banner.find('.accept-cookie ion-icon[name="close"], button, [aria-label*="close"], [aria-label*="fechar"]').first()
    ...
    fecharmodalpizza() {
        ...
        cy.get('body').then(($body) => {
            ...
            const temModalTexto = $body.text().toLowerCase().includes('parabéns') ||
                                  $body.text().toLowerCase().includes('pedir uma pizza')
                        ...
                        .then(($modal) => {
                            if ($modal.length > 0) {
                                ...
                                cy.wrap($modal).within(() => {
                                    cy.contains('button, ion-button, a, [role="button"]', 'Pedir uma pizza', { matchCase: false, timeout: 5000 })
                                        .should('exist')
    ...
    modalLojaFechada() {
        ...
        if (hora < 11) {
            ...
            cy.get('body').then(($body) => {
                ...
                const temModalLojaFechada = bodyText.includes('loja fechada') ||
                ...
                if (temModalLojaFechada) {
                    ...
                    cy.get('body').then(($body2) => {
                        ...
                        const temAppModal = $body2.find('app-modal-address-not-found, app-modal-loja-fechada, app-modal-store-closed, [class*="app-modal"]').length > 0
                        
                        // Verifica se existe ion-modal com classes específicas
                        const temModalEspecifica = $body2.find('ion-modal[class*="loja"], ion-modal[class*="fechada"], ion-modal[class*="store"], ion-modal.modal-default').length > 0
                        
                        if (temAppModal) {
                            // Estratégia 1: Componente app-modal-* específico (Opção 2)
                            cy.get('app-modal-address-not-found, app-modal-loja-fechada, app-modal-store-closed, [class*="app-modal"]', { timeout: 10000 })
                                .should('exist')
                                .within(() => {
                                    cy.contains('ion-button, button', 'Começar o meu pedido', { matchCase: false, timeout: 5000 })
                                        .should('be.visible')
                                        .click({ force: true })
                                        .then(() => {
                            ...
                            // Estratégia 2: ion-modal com classe específica (Opção 1)
                            cy.get('ion-modal[class*="loja"], ion-modal[class*="fechada"], ion-modal[class*="store"], ion-modal.modal-default', { timeout: 10000 })
                                .should('be.visible')
                                .within(() => {
                                    cy.contains('ion-button, button', 'Começar o meu pedido', { matchCase: false, timeout: 5000 })
                                        .should('be.visible')
                    ...
                    cy.get('body').then(($body3) => {
                        const temBotaoNaModal = $body3.find('ion-modal').find('button, ion-button').filter((i, el) => {
                        ...
                        if (!temBotaoNaModal) {
                            ...
                            cy.contains('button, ion-button, a, [role="button"]', /começar.*pedido/i, { timeout: 5000, matchCase: false })
                                .first()
                                .should('exist')
                                .scrollIntoView({ offset: { top: -100, left: 0 } })
                                .click({ force: true })
    ...
    clicarProduto() {
        ...
        // Não precisa de scroll complicado, o Cypress faz isso automaticamente com force: true
...
const fluxoCompraPage = new FluxoCompraPage()
export default fluxoCompraPage
